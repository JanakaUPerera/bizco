...
class LoginHistoryControllerSecurityTest {
    ...
    @Test
    void loginHistoryReadAllowedForUsersWithPermission() throws Exception {
        authenticate(Set.of("user.login_history.read"));
        when(loginHistoryService.search(null, 0, 20))
                .thenReturn(new LoginHistorySearchResponse(List.of(), 0, 20, 0, 0));

        mockMvc.perform(get("/api/v1/login-history").header("Authorization", "Bearer good-token"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data").isArray());
    }

    @Test
    void loginHistoryReadDeniedWithoutPermission() throws Exception {
