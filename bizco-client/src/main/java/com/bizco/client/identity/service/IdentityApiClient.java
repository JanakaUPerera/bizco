...
public class IdentityApiClient extends ApiClient {
    ...
    public CompletableFuture<List<UserResponse>> listUsers() {
        return get("/api/identity/users", new TypeReference<>() {
        });
    }

    public CompletableFuture<UserResponse> createUser(final UserCreateRequest request) {
        return post("/api/identity/users", request, new TypeReference<>() {
        });
    }

    public CompletableFuture<UserResponse> updateUser(final UUID userId, final UserUpdateRequest request) {
        return put("/api/identity/users/" + userId, request, new TypeReference<>() {
        });
    }

    public CompletableFuture<List<RoleResponse>> listRoles() {
        return get("/api/identity/roles", new TypeReference<>() {
        });
    }

    public CompletableFuture<RoleResponse> createRole(final RoleUpsertRequest request) {
        return post("/api/identity/roles", request, new TypeReference<>() {
        });
    }

    public CompletableFuture<RoleResponse> updateRole(final UUID roleId, final RoleUpsertRequest request) {
        return put("/api/identity/roles/" + roleId, request, new TypeReference<>() {
        });
    }

    public CompletableFuture<List<SecondaryRoleResponse>> listSecondaryRoles(final UUID userId) {
        return get("/api/identity/users/" + userId + "/secondary-roles", new TypeReference<>() {
        });
    }
    ...
    public CompletableFuture<SecondaryRoleResponse> grantSecondaryRole(
        ...
        return post("/api/identity/users/" + userId + "/secondary-roles", request, new TypeReference<>() {
        });
    }

    public CompletableFuture<Void> revokeSecondaryRole(final UUID grantId) {
        return delete("/api/identity/secondary-roles/" + grantId);
    }
}
